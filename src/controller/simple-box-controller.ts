...
export class SimpleBoxController {
  ...
  index(req: Request, res: Response) {
    ...
    const boxes = this.distributor.fillBoxes(request.boxSize, request.sheetSize);
    if (boxes.length === 0) {
      return errorResponse(res, new Error('Invalid sheet size. Too small for producing at least one box'));
    }
