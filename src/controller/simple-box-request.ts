...
export class SimpleBoxRequestValidator {
  validate(body: any): SimpleBoxRequest {
    if (typeof body['sheetSize'] !== 'object' || typeof body['boxSize'] !== 'object') {
      throw new Error('Invalid input format. Invalid request structure');
    }

    if (typeof body['sheetSize']['w'] !== 'number'
    ...
    if (
      ...
      throw new Error('Invalid input format.  Please use only positive integers');
    }
